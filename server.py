...
def appointment_page(id=0,type=0):
    ...
    print("?-"*26)
    print("request:",dict(request.form.items()))
    requests = dict(request.form.items())
...
def psychologist_page(id):
    ...
    if(request.method == "POST"):
        if("comment" in request.form):
            ...
            adds.append(id)
        elif(request.form.items()[0][1]=="delete"):
            com_id = request.form.items()[0][0]
            queries.delete("comment","comment.id = {}".format(com_id))
        elif("Cancel" in request.form.items()[0]):
            requests = dict(request.form.items())
            delete_one = requests.keys()[0]
            queries.delete("appointment","appointment.id={}".format(delete_one))    
        else:
            print("HA"*30,request.form.items())
            queries.update("comment","comment = '{}'".format(request.form.items()[0][1]),"id = {}".format(request.form.items()[1][0]) )
            queries.update("comment","comment = '{}'".format(request.form.items()[1][1]),"id = {}".format(request.form.items()[0][0]) )
